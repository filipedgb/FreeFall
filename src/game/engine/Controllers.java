...
/**
 * Esta classe representa os controlos do jogo, ou seja, verifica a interação
 * que o utilizador fez com o telemovel e faz as alterações necessárias ao jogo
 * 
...
public class Controllers implements OnTouchListener, SensorEventListener {
	...
	/**
	 * Se o jogador clica na metade direita do ecrã, o jogador move-se para a
	 * direita A aceleração tem um pico inicial (event ACTION_DOWN) e depois
	 * aumenta consoante o tempo que o jogador mantiver o ecrã premido
	 * 
	 * Quando o jogador levanta o dedo é desativada a flag hold e o jogador para
	 * de se mover.
	 * 
	 * Se o jogador clicar na parte de cima do ecrã, pode contrariar a
	 * gravidade, abrandando até poder mesmo começar a subir
	 * 
	 * Se o jogador clicar na parte de baixo do ecrã, acelera ainda mais
	 * ganhando pontos extra
	 * 
	 */
	@Override
	public boolean onTouch(View arg0, MotionEvent event) {
		if (current_game != null) {
			...
			if (event.getAction() == MotionEvent.ACTION_DOWN) {
				if (event.getY() < current_gameview.getHeight() / 6
						&& current_game.getPlayer().getFuel() > 0) {
				...
				else if (event.getY() > 5 * current_gameview.getHeight() / 6
						&& current_game.getPlayer().getFuel() > 0) {
					if (current_game.getPlayer().isMalfunctioning())
						current_game.setDirection(UP_DIRECTION);
					else {
	...
	public void onSensorChanged(SensorEvent event) {
		/**
		 * se o jogador tiver escolhido para usar acelerómetro controllerSensor
		 * = true
		 */
